...
########################
    ...
    # default settings
        ## 0. general stuff
        dataver = '5.3'
        # dataloc = '/Users/miamirkovic/Documents/UCB/crucible/MCD5.2/data/'
        dataloc = '/Users/aaronberliner/MCD/MCD5.3/data/'
    ...
    def convertlab(self,num):
        ...
        if num == "p": num = 91 #want this
        elif num == "rho": num = 92
        elif num == "t": num = 93 #want this
        elif num == "u": num = 94
        elif num == "v": num = 95
        ...
        elif num == "co2": #want this
            if "v5" in self.name:  num = 57
            else:                  num = 45
        elif num == "o3": #want this
...
###################
    ...
    # print requested space-time coordinates
        ...
        return (self.lat, self.lon, self.loct, self.xdate)
...
###################
    ...
    ### complete 1D figure with possible multiplots
        import mcdcomp2 as mcdcomp2
        if isinstance(tabtodo,np.str): tabtodo=[tabtodo] ## so that asking one element without [] is possible.
        if isinstance(tabtodo,np.int): tabtodo=[tabtodo] ## so that asking one element without [] is possible.
        fig = mpl.figure() ; subv,subh = mcdcomp2.definesubplot( len(tabtodo) , fig )
    ...
    ### see http://www.dalkescientific.com/writings/diary/archive/2005/04/23/matplotlib_without_gui.html
        ...
        subv,subh = mcdcomp2.definesubplot( len(tabtodo) , fig )
        for i in range(len(tabtodo)):
...
###################
    ...
    #added this function to keep lt fixed
    def latlon2(self,ndx=37,ndy=19):
        self.fixedlt = True
        self.latlon(ndx,ndy)
    ...
    def makemap2d(self,choice,x=None,y=None,incwind=False,proj="cyl"):
    ...
    # STRIPPED DOWN
        ...
        if choice == "wind":
            ...
            (field, fieldlab) = self.definefield(choice)
        if incwind:   ret = mcdcomp2.maplatlon(self.xcoord,self.ycoord,field,title=fieldlab,proj=proj,vecx=windx,vecy=windy) #,stride=1)
        else:         ret = mcdcomp2.maplatlon(self.xcoord,self.ycoord,field,title=fieldlab,proj=proj)
    ...
    ### complete 2D figure with possible multiplots
        import mcdcomp2 as mcdcomp2
        if isinstance(tabtodo,np.str): tabtodo=[tabtodo] ## enables request of one element instead of default array
        if isinstance(tabtodo,np.int): tabtodo=[tabtodo] ## enables request of one element instead of default array
        fig = mpl.figure()
        subv,subh = mcdcomp2.definesubplot( len(tabtodo) , fig )
        for i in range(len(tabtodo)): mpl.subplot(subv,subh,i+1) ; ret = self.makemap2d(tabtodo[i],incwind=incwind,proj=proj)
        mpl.show()

    def map2d_getdata(self,tabtodo,incwind=False,proj="cyl"):
    ### complete 2D figure with possible multiplots
    ### EDIT: TO COMPUTE FIGURE, UNCOMMENT THE LINES IN THIS FILE AND MCDCOMP2 WITH #UFF#
        import mcdcomp2 as mcdcomp2
        if isinstance(tabtodo,np.str): tabtodo=[tabtodo] ## so that asking one element without [] is possible.
        if isinstance(tabtodo,np.int): tabtodo=[tabtodo] ## so that asking one element without [] is possible.
        final = self.makemap2d(tabtodo[0],incwind=incwind,proj=proj)
        x = final[0]
        y = final[1]
        fig = mpl.figure();
        subv,subh = mcdcomp2.definesubplot( len(tabtodo) , fig );

        for i in range(len(tabtodo) - 1):
            mpl.subplot(subv,subh,i+1)
            final.append(self.makemap2d(tabtodo[i+1],x,y,incwind=incwind,proj=proj)[2])
        return fig,final
        return final
    ...
    ### see http://www.dalkescientific.com/writings/diary/archive/2005/04/23/matplotlib_without_gui.html
        ...
        fig = mcdcomp2.setfig(len(tabtodo),proj=self.proj)
        subv,subh = mcdcomp2.definesubplot( len(tabtodo) , fig )
        ...
        for i in range(len(tabtodo)):
            ...
            zevmin,zevmax,limtype = mcdcomp2.setbounds(what_I_plot,vmin=self.min2d,vmax=self.max2d)
    ...
    ### see http://www.dalkescientific.com/writings/diary/archive/2005/04/23/matplotlib_without_gui.html
        ...
        fig = mcdcomp2.setfig(len(tabtodo))
        subv,subh = mcdcomp2.definesubplot( len(tabtodo) , fig )
